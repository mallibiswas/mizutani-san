...
def calculateMetrics (_df, _aggList):
    ...
    _df_reshaped['Mean'] = _df_reshaped['NormalizedSales']/_df_reshaped['StoreCount']
...
def calculateKPIs (_df, _aggList):
    ...
    idnames = ['CampaignKey','MethodologyKey','Methodology'] + _aggList + ['Measure']
#    _df_smry = _df.pivot_table(index=idnames, columns='ClusterName', aggfunc=np.sum)
    ...
    normalized_df =  _df_smry.iloc[_df_smry.index.get_level_values('Methodology') != 'Campaign Raw']
    ...
    raw_df_ ["IS"]   = raw_df_['TCAS']/(1-1/raw_df_['LiftMultiplier'])
    ...
    raw_df_.drop([_aggList[1],'Methodology'], axis=1, inplace=True)
    
#    Merge on CampaignKey, ProductKey, Measure
    df_ = normalized_df_.merge(raw_df_,  left_on=['CampaignKey']+[_aggList[0]]+['Measure'], \
                                            right_on=['CampaignKey']+[_aggList[0]]+['Measure'], how='inner')
...
def outputKPIs (_df, _pv, _aggList):
    ...
    #    Merge on CampaignKey, ProductKey, Measure
    df_ = _df.merge(_pv,  left_on=['CampaignKey']+[_aggList[0]]+['MethodologyKey','Measure'], \
                                            right_on=['CampaignKey']+[_aggList[0]]+['MethodologyKey','Measure'], how='inner')
